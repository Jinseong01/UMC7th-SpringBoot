...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 사용자 정보 저장 또는 업데이트
        UserEntity user = saveOrUpdateUser(email, nickname);
    ...
    private UserEntity saveOrUpdateUser(String email, String nickname) {
        UserEntity member = userRepository.findByEmail(email)
                .orElse(UserEntity.builder()
                        .email(email)
                        .name(nickname)
                        .login_password(passwordEncoder.encode("OAUTH_USER_" + UUID.randomUUID()))
                        .gender(UserGender.NONE)  // 기본값 설정
